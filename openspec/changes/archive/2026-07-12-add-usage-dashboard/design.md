...
- **Token usage** is counted once per distinct `(messageID, requestID)`, taking the `usage` of the line that carries a **`stop_reason`** — the line that ended the response. Taking the first line undercounts output by 8% (965 252 against a true 1 048 659). Position is not the rule: "last line wins" happens to agree with the data today, but it would silently pick a placeholder again if lines were ever reordered or a file concatenated. A response that never gains a stop reason — an interruption — falls back to its last line. The response's timestamp and working directory still come from its first line, because that is when it began.
